...
def generate_candl_file_from_template(candl_file_template_text, graph):
    """
    ...
    """
    func_string = graph_to_func(graph, unit_string='u', neighbour_sring='n')
    if '$FUNC$' not in candl_file_template_text:
        raise Exception("$FUNC$ not found in candl_file_template_text")
    return candl_file_template_text.replace('$FUNC$', f' {func_string} ')
