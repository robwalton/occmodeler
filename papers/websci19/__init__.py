...
def create_run_on_tos_network_function(calling_file):

    def run_on_tos_network(
            model, medium_graph=MEDIUM_GRAPH, graph_name='tos network',
            start=0, stop=20, step=.1, runs=1,
            calling_file=calling_file, save_run=False):
        return call.run_experiment(
            model, medium_graph=medium_graph, graph_name=graph_name,
            start=start, stop=stop, step=step, runs=runs,
